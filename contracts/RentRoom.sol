...
contract RentRoom is ReentrancyGuard, Ownable {
        ...
        nonReentrant()
    {
        ...
        if(msg.value < (roomNft.pricePerDay * _periodInDays)) {
            revert RentRoom__PriceNotMet(_tokenId, roomNft.pricePerDay * _periodInDays);
        }

        uint64 period = daysToUnix(_periodInDays);

        uint256 fee = (msg.value * _rentingFeePercentage / 100);
        totalFees += fee;
        proceeds[IERC721(roomTokenContractAddress).ownerOf(_tokenId)] += (msg.value - fee);
        delete(listings[_tokenId]);
    ...
    function withdrawFees() external onlyOwner {
        ...
        totalFees = 0;
        (bool success, ) = payable(msg.sender).call{value: _totalFees}("");
        require(success, "Transfer failed");
    }
